...
namespace ESFA.DC.ILR.FundingService.FundingActor.Tasks
{
    public class FM81ActorTask : IActorTask
    {
        ...
        private readonly IActorProvider<IFM81Actor> _fundingActorProvider;
        private readonly IFilePersistanceService _filePersistanceService;
        private readonly IFundingOutputCondenserService<FM81Global> _fundingOutputCondenserService;
        private readonly string _actorName;

        public FM81ActorTask(
            IJsonSerializationService jsonSerializationService,
            IActorProvider<IFM81Actor> fundingActorProvider,
            IFilePersistanceService filePersistanceService,
            IFundingOutputCondenserService<FM81Global> fundingOutputCondenserService,
            ILogger logger,
            string actorName)
        {
        ...
        public async Task Execute(IEnumerable<FundingActorDto> fundingActorDtos, IFundingServiceContext fundingServiceContext, CancellationToken cancellationToken)
        {
            ...
            List<IFM81Actor> actors = new List<IFM81Actor>();

            foreach (FundingActorDto fundingActorDto in fundingActorDtos)
            {
                IFM81Actor actor = _fundingActorProvider.Provide();
                actors.Add(actor);
            ...
            IEnumerable<FM81Global> results = taskList.Select(t => _jsonSerializationService.Deserialize<FM81Global>(t.Result));
            ...
            foreach (IFM81Actor actor in actors)
            {
